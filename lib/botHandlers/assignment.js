...
const { getStatusEmoji, getProgressStatusEmoji, getStageEmoji, getStageStatusEmoji } = require('../botUtils');
  ...
  try {
    ...
    for (const stage of stages) {
      ...
      if (key && progressRecord && progressRecord[key]) {
        ...
        if (st) { statusEmoji = getProgressStatusEmoji(st); statusText = st; }
        else { statusText = 'Belum ada progress'; }
      } else if (key === null) { statusText = 'Belum ada progress'; }

      if (assignment) {
        const techName = assignment.users?.name || 'Unknown';
        message += `${stageEmoji} ${stage}: ${statusEmoji} ${statusText} • ${techName}\n`;
        keyboard.push([{ text: `🔄 Reassign ${stage}`, callback_data: `reassign_stage_${order.order_id}_${stage}` }]);
      } else {
        message += `${stageEmoji} ${stage}: ${statusEmoji} ${statusText}\n`;
        keyboard.push([{ text: `➕ Assign ${stage}`, callback_data: `assign_stage_${order.order_id}_${stage}` }]);
      }
    }
  ...
  try {
    ...
    if (existingAssignment) {
      const { error: updateError } = await supabase
        .from('order_stage_assignments')
        .update({ assigned_technician: String(techId), assigned_by: String(telegramId), assigned_at: new Date().toISOString(), status: 'assigned' })
        .eq('id', existingAssignment.id);
      if (updateError) { await client.sendMessage(chatId, '❌ Gagal mengupdate assignment.'); return; }
    } else {
      const { error: insertError } = await supabase
        .from('order_stage_assignments')
        .insert({ order_id: order.order_id, stage, assigned_technician: String(techId), assigned_by: String(telegramId), assigned_at: new Date().toISOString(), status: 'assigned' });
    ...
    await client.sendMessage(chatId, confirmMsg, { parse_mode: 'Markdown' });
    const techMessage = `🔔 Assignment Baru!\n\n${stageEmoji} Stage: ${stage}\n📋 Order: ${order.order_id}\n👤 Customer: ${order.customer_name}\n📍 STO: ${order.sto}\n\nAnda telah di-assign untuk menangani stage ini.`;
    await client.sendMessage(Number(techId), techMessage, { parse_mode: 'Markdown' });
  ...
  try {
    ...
    for (const stage of stages) {
      ...
      if (existingAssignment) {
        ...
        const { error: insertError } = await supabase
          .from('order_stage_assignments')
          .insert({ order_id: orderId, stage, assigned_technician: String(techId), assigned_by: String(telegramId), assigned_at: new Date().toISOString(), status: 'assigned' });
        if (insertError) errorCount++; else successCount++;
      }
    }

    const { data: order } = await supabase
      .from('orders').select('order_id, customer_name, sto').eq('order_id', orderId).maybeSingle();
    ...
    let confirmMessage = `✅ Bulk Assignment Selesai!\n\n` +
      ...
      `\nTeknisi telah diberi notifikasi.`;
    await client.sendMessage(chatId, confirmMessage, { parse_mode: 'Markdown' });
    ...
    const techMessage = `🔔 Bulk Assignment Baru!\n\n` +
      ...
      `👤 Customer: ${order?.customer_name || 'N/A'}\n` +
      `📍 STO: ${order?.sto || 'N/A'}\n\n` +
      `Anda telah di-assign untuk semua stage dalam order ini.`;
    await client.sendMessage(Number(techId), techMessage, { parse_mode: 'Markdown' });
