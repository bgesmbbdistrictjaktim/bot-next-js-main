...
const { getStatusEmoji, getStageEmoji, getStageStatusEmoji, formatIndonesianDateTime } = require('../botUtils');
...
async function showTechnicianStageAssignments(client, chatId, telegramId) {
  try {
    ...
    Object.keys(orderGroups).forEach(orderId => {
      keyboard.push([{ text: `🔄 Update Progress - ${orderId}`, callback_data: `tech_stage_progress_${orderId}` }]);
    });
