  ...
  try {
    ...
    if (/Masukkan ORDER ID baru:/i.test(replyText)) {
      ...
      const insertPayload = {
        ...
        status: 'Pending',
        created_by: user.id,
        // Prefill NOT NULL columns to avoid constraint violations
        customer_name: 'Belum diisi',
        customer_address: 'Belum diisi',
        contact: '-',
        // Optional fields that may exist in schema
        sto: 'Belum diisi',
        transaction_type: 'Belum diisi',
        service_type: 'Belum diisi',
      };
    ...
    if (/Masukkan Jenis Transaksi untuk ORDER/i.test(replyText) && orderId) {
      await supabase
        ...
        .eq('order_id', orderId);
      await client.sendMessage(chatId, `Masukkan Jenis Layanan untuk ORDER ${orderId}:`, { reply_markup: { force_reply: true } });
      return true;
    }
