...
async function startCreateOrderFlow(client, chatId, telegramId) {
  try {
    ...
    await client.sendMessage(chatId, '📋 Membuat Order Baru\nGunakan tombol di bawah untuk kembali atau batalkan.', {
      reply_markup: { inline_keyboard: [[{ text: '⬅️ Kembali', callback_data: 'back_to_hd_menu' }], [{ text: '❌ BATALKAN / END SESSION', callback_data: 'cancel_session' }]] }
    });
    await client.sendMessage(chatId, '🆕 Masukkan ORDER ID baru:', { reply_markup: { force_reply: true } });
  } catch (error) {
...
async function handleCreateOrderReply(client, chatId, telegramId, replyText, text) {
  try {
    ...
    if (/Masukkan ORDER ID baru:/i.test(replyText)) {
      ...
      if (!orderId) {
        await client.sendMessage(chatId, 'Gunakan tombol di bawah untuk kembali atau batalkan.', { reply_markup: { inline_keyboard: [[{ text: '⬅️ Kembali', callback_data: 'back_to_hd_menu' }], [{ text: '❌ BATALKAN / END SESSION', callback_data: 'cancel_session' }]] } });
    ...
    if (/Masukkan Jenis Transaksi untuk ORDER/i.test(replyText) && orderId) {
      ...
      await client.sendMessage(chatId, 'Gunakan tombol di bawah untuk kembali atau batalkan.', { reply_markup: { inline_keyboard: [[{ text: '⬅️ Kembali', callback_data: 'back_to_hd_menu' }], [{ text: '❌ BATALKAN/EN SESSION', callback_data: 'cancel_session' }]] } });
