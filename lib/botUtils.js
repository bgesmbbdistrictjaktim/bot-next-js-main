...
// Consistent newest-first sorting for order arrays across menus.
// Primary: created_at desc; Fallback: numeric part of order_id desc; Stable for ties.
function sortOrdersNewestFirst(orders = []) {
  const toNumId = (oid) => {
    const n = Number(String(oid || '').replace(/\D/g, ''));
  ...
  return orders.slice().sort((a, b) => {
    ...
    const na = toNumId(a?.order_id || a?.id);
    const nb = toNumId(b?.order_id || b?.id);
    return nb - na;
  });
}
